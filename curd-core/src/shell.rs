...
const FOREGROUND_SHELL_TIMEOUT_SECS: u64 = 60;
const MAX_ACTIVE_BACKGROUND_TASKS: usize = 16;
...
fn read_locked_string(buffer: &Arc<std::sync::Mutex<String>>) -> String {
    buffer.lock().map(|g| g.clone()).unwrap_or_default()
}

fn read_locked_exit_code(code: &Arc<std::sync::Mutex<Option<i32>>>) -> Option<i32> {
    code.lock().map(|g| *g).unwrap_or(None)
}
...
impl ShellEngine {
    ...
    pub async fn shell(
        ...
        if is_background && !self.policy.exec.allow_background {
            anyhow::bail!("Background execution is disabled by [policy.exec].");
        }
        ...
        let cwd = self.validate_cwd_override(cwd_override)?;
        let mut cmd = if cfg!(target_os = "windows") {
            let mut c = self
                .sandbox
                .build_command("cmd", &["/C".to_string(), command.to_string()]);
            c.current_dir(cwd);
            c
        } else {
        ...
        let mut stdout = child
            .stdout
            .take()
            .ok_or_else(|| anyhow::anyhow!("Shell process stdout pipe unavailable"))?;
        let mut stderr = child
            .stderr
            .take()
            .ok_or_else(|| anyhow::anyhow!("Shell process stderr pipe unavailable"))?;
        ...
        if is_background {
            let active_background = self
                ...
                .count();
            if active_background >= MAX_ACTIVE_BACKGROUND_TASKS {
                let _ = child.kill().await;
                anyhow::bail!(
                    "Background task limit exceeded ({} active tasks).",
            ...
            tokio::spawn(async move {
                ...
                while let Ok(n) = stdout.read(&mut buf).await {
                    ...
                    let Ok(mut guard) = out_arc.lock() else {
                        break;
                    };
                    guard.push_str(&s);
                    if guard.len() > 1024 * 512 {
                        break;
                    }
                }
            });
            ...
            tokio::spawn(async move {
                ...
                while let Ok(n) = stderr.read(&mut buf).await {
                    ...
                    let s = String::from_utf8_lossy(&buf[..n]);
                    let Ok(mut guard) = err_arc.lock() else {
                        break;
                    };
                    guard.push_str(&s);
                    if guard.len() > 1024 * 512 {
        ...
        let status = match tokio::time::timeout(
            std::time::Duration::from_secs(FOREGROUND_SHELL_TIMEOUT_SECS),
            child.wait(),
        )
        .await
        {
            ...
            Err(_) => {
                ...
                return Err(anyhow::anyhow!(
                    "Command exceeded foreground timeout of {}s.",
                    FOREGROUND_SHELL_TIMEOUT_SECS
                ));
            }
        };
    ...
    pub async fn status(&self, task_id: Uuid) -> Result<ShellTaskStatus> {
        ...
        if let Some(mut child) = state.child.take() {
            match child.try_wait() {
                Ok(Some(status)) => {
                    if let Ok(mut exit_code) = state.exit_code.lock() {
        ...
        let exit_code = read_locked_exit_code(&state.exit_code);
        Ok(ShellTaskStatus {
            task_id,
            command: state.command.clone(),
            active: exit_code.is_none(),
            exit_code,
            stdout: read_locked_string(&state.stdout_buf),
            stderr: read_locked_string(&state.stderr_buf),
        })
    }
    ...
    pub fn validate_command(&self, command: &str) -> Result<()> {
        ...
        for bad in &bad_chars {
            if command.contains(bad) {
                anyhow::bail!(
                    "Command chaining or subshells ('{}') are forbidden in the sandbox.",
                    bad
                );
            }
        }
        ...
        if !self.policy.allowed_binaries.is_empty() {
            ...
            let default_allowed = [
                "cargo", "npm", "yarn", "pnpm", "bun", "python", "python3", "pytest", "node",
                "make", "ninja", "cmake", "go", "gcc", "clang", "g++", "clang++", "rustc", "tsc",
                "jest", "vitest", "npx", "echo", "sleep", "cat", "ls", "grep", "curd",
            ];
            if !default_allowed.contains(&program) {
                anyhow::bail!(
                    "Command '{}' is not in the default allowed binaries list.",
                    program
                );
            }
        }
    ...
    fn validate_cwd_override(&self, cwd_override: Option<&Path>) -> Result<PathBuf> {
        let cwd = cwd_override.unwrap_or(&self.workspace_root);
        let cwd = std::fs::canonicalize(cwd).unwrap_or_else(|_| cwd.to_path_buf());
        if !cwd.starts_with(&self.workspace_root) {
            anyhow::bail!(
                "Shell cwd override escaped the workspace root: {}",
                cwd.display()
            );
        }
        Ok(cwd)
...
const SHELL_NOISE_WORDS: [&str; 11] = [
    "error", "warning", "note", "info", "failed", "failure", "panic", "thread", "exit", "stdout",
    "stderr",
];
...
mod tests {
    ...
    #[tokio::test]
    async fn test_shell_background_respects_policy() {
        let dir = tempdir().unwrap();
        let policy = crate::policy::PolicyConfig {
            exec: crate::policy::PolicyExecConfig {
                allow_raw_command: false,
                allow_background: false,
            },
            ..Default::default()
        };
        let engine = ShellEngine::new(dir.path(), Default::default(), policy);
        let err = engine
            .shell("sleep 1", None, true)
            .await
            .expect_err("background should be denied");
        assert!(err.to_string().contains("Background execution is disabled"));
    }

    #[tokio::test]
    async fn test_shell_rejects_cwd_override_outside_workspace() {
        let dir = tempdir().unwrap();
        let outside = tempdir().unwrap();
        let engine = ShellEngine::new(dir.path(), Default::default(), Default::default());
        let err = engine
            .shell("echo hello", Some(outside.path()), false)
            .await
            .expect_err("cwd override should be denied");
        assert!(err.to_string().contains("escaped the workspace root"));
    }

    #[tokio::test]
    async fn test_shell_background_task_limit() {
        let dir = tempdir().unwrap();
        let engine = ShellEngine::new(dir.path(), Default::default(), Default::default());
        for _ in 0..MAX_ACTIVE_BACKGROUND_TASKS {
            let res = engine.shell("sleep 5", None, true).await.unwrap();
            assert_eq!(res.get("status").unwrap().as_str().unwrap(), "background");
        }
        let err = engine
            .shell("sleep 5", None, true)
            .await
            .expect_err("background limit should be enforced");
        assert!(err.to_string().contains("Background task limit exceeded"));
    }
    ...
    async fn test_shell_graph_context_uses_indexed_symbols() {
        ...
        assert!(
            res["graph_context"]["graph"]["detailed_edges"]
                .as_array()
                .is_some()
        );
    }
...
pub fn parse_command(command: &str) -> Result<(String, Vec<String>)> {
    ...
    let program = it
        .next()
        .ok_or_else(|| anyhow::anyhow!("Missing program in command"))?;
