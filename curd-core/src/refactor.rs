use crate::{GraphEngine, SearchEngine, ShadowStore, run_build, BuildRequest};
use anyhow::Result;
use std::path::{Path, PathBuf};

#[derive(Clone)]
pub enum RefactorAction {
    Rename { symbol: String, new_name: String, lsp_binary: Option<String> },
    Move { symbol: String, target_file: PathBuf },
    Extract { file_range: String, new_function_name: String },
}
...
pub fn run_refactor(root: &Path, action: RefactorAction) -> Result<String> {
    ...
    let shadow_root_path = shadow.shadow_root.clone().ok_or_else(|| anyhow::anyhow!("Failed to initialize shadow root"))?;
    ...
    match action {
        RefactorAction::Rename { symbol, new_name, mut lsp_binary } => {
            if symbol.starts_with('@') {
                return Err(anyhow::anyhow!("Refactoring symbols inside linked external contexts is prohibited."));
            }
            ...
            if lsp_binary.is_none() {
                ...
                let has_line_hint = parts.len() >= 3 && parts[parts.len()-2].parse::<usize>().is_ok() && parts[parts.len()-1].parse::<usize>().is_ok();
                    ...
                    if symbols.len() > 1 && !has_line_hint {
                        ...
                        for target in symbols.iter() {
                            ...
                            if let Ok(src) = std::fs::read_to_string(&path) {
                                ...
                                let symbol_text = &src[target.start_byte..target.end_byte.min(src.len())];
                                let signature = symbol_text.lines().next().unwrap_or("").trim_end_matches(" {").trim_end_matches('{').trim();
                                options.push(format!("{} (line {}): {}", target.filepath.display(), line, signature));
                            }
                        }
                        return Err(anyhow::anyhow!("Symbol '{}' is overloaded. Please disambiguate by appending the line number to the URI (e.g., '{}::LINE_NUMBER').\nAvailable options:\n{}", symbol_name, symbol, options.join("\n")));
                         ...
                         let file_str = symbol[..symbol.len() - parts[parts.len()-1].len() - parts[parts.len()-2].len() - 2].to_string();
                         let target_file = root.join(file_str);
                         let ext = target_file.extension().and_then(|e| e.to_str()).unwrap_or("").to_lowercase();
                         ...
                         if let Some(cmd) = auto_lsp {
                                out.push_str(&format!("Auto-starting '{}' for safe renaming of specific overload...\n", cmd));
            ...
            if let Some(lsp_cmd) = lsp_binary {
                out.push_str(&format!("Using LSP ({}) to rename `{}` to `{}`...\n", lsp_cmd, symbol, new_name));
                ...
                if lsp_auto_detected && chosen_target_file.exists() {
                    ...
                    let parts: Vec<&str> = symbol.split(':').collect();
                    if parts.len() >= 3 && parts[parts.len()-2].parse::<usize>().is_ok() && parts[parts.len()-1].parse::<usize>().is_ok() {
                ...
                if let Some(changes) = edit_result.get("changes").and_then(|v| v.as_object()) {
                    for (uri, edits) in changes {
                        let rel_path = uri.replace(&format!("file://{}", root.display()), "").trim_start_matches('/').to_string();
                        if crate::workspace::validate_sandboxed_path(root, &rel_path).is_err() { continue; } // Sandbox defense
                        
                        let abs_path = root.join(&rel_path);
                        let shadow_file = shadow_root_path.join(&rel_path);
                        
                        let mut content = std::fs::read_to_string(&shadow_file).or_else(|_| std::fs::read_to_string(&abs_path))?;
                ...
                } else if let Some(doc_changes) = edit_result.get("documentChanges").and_then(|v| v.as_array()) {
                ...
                out.push_str(&format!("Renaming symbol `{}` to `{}` (Greedy AST mode)...\n", symbol, new_name));
                
                // Collect all targets to modify (definition + callers)
                // Storing (start_byte, end_byte, old_name_to_replace)
                let mut modifications: std::collections::HashMap<PathBuf, Vec<(usize, usize, String)>> = std::collections::HashMap::new();
                ...
                if let Some(target) = symbols.first() {
                    modifications.entry(target.filepath.clone()).or_default().push((target.start_byte, target.end_byte, old_name.clone()));
                }
                ...
                for caller_id in callers {
                    if caller_id.starts_with('@') {
                        ...
                        if let Some(idx) = caller_id.find("::") {
                            ...
                            if let Some(link) = registry.contexts.get(alias) {
                                if link.mode == crate::context_link::ContextMode::Write {
                                    ...
                                    let alert_msg = format!("WARNING: Symbol `{}` calls `{}` which was just renamed to `{}` in an upstream workspace.", caller_id, symbol, new_name);
                                    ...
                                    out.push_str(&format!("  [EXTERNAL SKIP] {} (Warning appended to {})", alert_msg, alert_path.display()));
                                }
                            }
                        }
                        ...
                        if let Some(caller) = c_symbols.first() {
                            ...
                            let abs_ext_path = ext_root.join(caller.filepath.strip_prefix(&ext_root).unwrap_or(&caller.filepath));
                            modifications.entry(abs_ext_path).or_default().push((caller.start_byte, caller.end_byte, old_target_name.clone()));
                        }
                        continue;
                    }
                    ...
                    if let Some(caller) = c_symbols.first() {
                        ...
                        modifications.entry(caller.filepath.clone()).or_default().push((caller.start_byte, caller.end_byte, old_target_name.clone()));
                    }
                }
            ...
            let build_resp = run_build(&shadow_root_path, BuildRequest {
                adapter: None,
                profile: None,
                target: None,
                execute: true,
        ...
        RefactorAction::Move { symbol, target_file } => {
            if symbol.starts_with('@') {
                return Err(anyhow::anyhow!("Refactoring symbols inside linked external contexts is prohibited."));
            }

            out.push_str(&format!("Moving symbol `{}` to `{}`...\n", symbol, target_file.display()));
            ...
            if let Some(target) = symbols.first() {
                ...
                if let Ok(source) = std::fs::read_to_string(&source_file_path) {
                    ...
                    let shadow_source = std::fs::read_to_string(&shadow_source_file).unwrap_or(source.clone());
                    ...
                    let mut target_content = std::fs::read_to_string(&shadow_target_file).or_else(|_| std::fs::read_to_string(&absolute_target)).unwrap_or_default();
        ...
        RefactorAction::Extract { file_range, new_function_name } => {
            if file_range.starts_with('@') {
                return Err(anyhow::anyhow!("Refactoring symbols inside linked external contexts is prohibited."));
            }

            out.push_str(&format!("Extracting `{}` to `{}`...\n", file_range, new_function_name));
            let parts: Vec<&str> = file_range.split(':').collect();
            if parts.len() == 2 {
                let file_path = PathBuf::from(parts[0]);
                let range_parts: Vec<&str> = parts[1].split('-').collect();
                if range_parts.len() == 2 {
                    if let (Ok(start_line), Ok(end_line)) = (range_parts[0].parse::<usize>(), range_parts[1].parse::<usize>()) {
                        let absolute_file = root.join(&file_path);
                        let shadow_file = shadow_root_path.join(&file_path);
                        if let Ok(source) = std::fs::read_to_string(&shadow_file).or_else(|_| std::fs::read_to_string(&absolute_file)) {
...
fn apply_lsp_edits(content: String, edits: &[serde_json::Value]) -> Result<String> {
    ...
    for e in edits {
        ...
        let start_line = range.get("start").unwrap().get("line").unwrap().as_u64().unwrap() as usize;
        let start_col = range.get("start").unwrap().get("character").unwrap().as_u64().unwrap() as usize;
        let end_line = range.get("end").unwrap().get("line").unwrap().as_u64().unwrap() as usize;
        let end_col = range.get("end").unwrap().get("character").unwrap().as_u64().unwrap() as usize;
    ...
    for (sl, sc, el, ec, new_text) in parsed_edits {
        ...
        if sl == el {
            ...
            let end_line_text = if el < lines.len() { &lines[el][ec.min(lines[el].len())..] } else { "" };
    ...
    Ok(lines.into_iter().filter(|s| !s.is_empty() || edits.is_empty()).collect::<Vec<_>>().join("\n"))
}
...
mod tests {
    ...
    fn setup_workspace(root: &Path) {
        ...
        if let Ok(cwd) = std::env::current_dir() {
            ...
            let src = if real_grammars.exists() { real_grammars } else { real_grammars_fallback };
            if src.exists() {
...
";
        ...
        println!("All indexed symbols: {:?}", all_syms.iter().map(|s| s.id.clone()).collect::<Vec<_>>());
        ...
        let _out = run_refactor(root, action).unwrap_or_else(|e| panic!("run_refactor failed: {}", e));
        ...
        assert_eq!(calls, 3, "Expected 2 caller instances + 1 def to be updated, got:\n{}", content);
    }
    ...
    fn test_refactor_move() {
        ...
        let out = run_refactor(root, action).unwrap_or_else(|e| panic!("run_refactor failed: {}", e));
        assert!(out.contains("Move complete"));
        
        let mut shadow = ShadowStore::new(root);
        shadow.begin().unwrap();
        
        let a_content = fs::read_to_string(shadow.shadow_root.as_ref().unwrap().join("src/a.py")).unwrap();
        if a_content.contains("def foo():") {
            panic!("Symbol was not removed! Content is:\n{}", a_content);
        }
        assert!(!a_content.contains("def foo():"));
        
        let b_content = fs::read_to_string(shadow.shadow_root.as_ref().unwrap().join("src/b.py")).unwrap();
