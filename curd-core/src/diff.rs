...
pub fn run_diff(root: &Path, semantic: bool, target_symbol: Option<String>) -> Result<String> {
    ...
    let shadow_root = shadow.shadow_root.clone().unwrap();
    ...
    for file_path in staged_paths {
        ...
        let orig_symbols = if file_path.exists() {
            search.parse_file(&file_path, &mut manager).unwrap_or_default()
        } else {
            vec![]
        };

        let new_symbols = if shadow_path.exists() {
            search.parse_file(&shadow_path, &mut manager).unwrap_or_default()
        } else {
            vec![]
        };

        let mut orig_map: HashMap<String, Symbol> = orig_symbols.into_iter().map(|s| {
            let relative_path = file_path.strip_prefix(root).unwrap_or(&file_path).to_string_lossy();
            let key = format!("{}::{}", relative_path, s.name);
            (key, s)
        }).collect();
        let mut new_map: HashMap<String, Symbol> = new_symbols.into_iter().map(|s| {
            let relative_path = shadow_path.strip_prefix(&shadow_root).unwrap_or(&shadow_path).to_string_lossy();
            let key = format!("{}::{}", relative_path, s.name);
            (key, s)
        }).collect();
            ...
            match (orig_map.get(&key), new_map.get(&key)) {
                (Some(o), Some(n)) => {
                    if o.semantic_hash != n.semantic_hash {
                        out.push_str(&format!("function `{}`: modified (AST differences detected)\n", key));
                    }
                }
                (Some(_o), None) => {
...
mod tests {
    ...
    fn test_run_diff_semantic_modifications() {
        ...
        let mut shadow = ShadowStore::new(&root);
        shadow.begin().unwrap();
        shadow.stage(&file_path, "def foo():\n    return 2\n").unwrap();
