...
async fn test_tool_registry_dispatch() -> Result<()> {
    ...
    let open_res =
        curd_core::context::dispatch_tool("session_open", &json!({"pubkey_hex": pubkey_hex}), &ctx)
            .await;
    let nonce = open_res
        .get("nonce")
        .expect(&format!("nonce missing: {:?}", open_res))
        .as_str()
        .unwrap();
    ...
    let verify_res = curd_core::context::dispatch_tool(
        "session_verify",
        &json!({
    ...
    let connection_token = verify_res
        .get("connection_token")
        .expect(&format!("token missing: {:?}", verify_res))
        .as_str()
        .unwrap();

    // 2. Verify a simple stateless tool (stamina)
    let res = curd_core::context::dispatch_tool(
        "stamina",
        &json!({"connection_token": connection_token}),
        &ctx,
    )
    .await;
    assert!(
        res.get("budget").is_some(),
        "Stamina tool should return a budget object. Result: {:?}",
        res
    );
    ...
    let res = curd_core::context::dispatch_tool(
        "read",
        &json!({"uris": ["nonexistent.rs"], "connection_token": connection_token}),
        &ctx,
    )
    .await;
    let results = res.get("results").unwrap().as_array().unwrap();
    assert!(
        results.is_empty() || results[0].get("error").is_some(),
        "Reading nonexistent file should return empty or error"
    );
