...
async fn test_vscode_stress_operations() {
    ...
    if !std::path::Path::new(workspace_root).exists() {
        println!(
            "Skipping vscode stress test because {} does not exist.",
            workspace_root
        );
        return;
    }
    ...
    let symbols = se
        .search("registerCommand", None)
        .expect("Failed to search");
    let idx_duration = start_idx.elapsed();
    println!(
        "Initial search/indexing took: {:?}. Found {} 'registerCommand' functions.",
        idx_duration,
        symbols.len()
    );
    ...
    let symbol_id = symbols.first().map(|s| s.id.clone()).unwrap_or_else(|| {
        "src/vs/workbench/api/common/extHostCommands.ts::registerCommand".to_string()
    });

    println!("Graph Tree Search for {}...", symbol_id);
    let start_tree = Instant::now();
    let tree = ge
        .graph_with_depths(vec![symbol_id.clone()], 3, 3)
        .expect("Failed graph tree");
    println!(
        "Tree Search (3 up, 3 down) completed in: {:?}",
        start_tree.elapsed()
    );
    ...
    let candidates = crawl_result
        .get("ranked_candidates")
        .and_then(|c| c.as_array());
    println!(
        "  Crawl yielded {} candidates",
        candidates.map(|c| c.len()).unwrap_or(0)
    );
